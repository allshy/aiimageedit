...
- 支持 `原图优先`、`兼容补边`、`高质JPG`、`无损PNG`、`原图` 五种上传模式
- `原图优先` 会在图片长边不超过 4096 时直接上传原图；超过 4096 时才等比缩小到所选长边，不补边
- `兼容补边` 用于接口不稳定或特殊尺寸，会自动识别 `9:16 / 2:3 / 3:4 / 1:1 / 4:3 / 3:2 / 16:9` 等常见比例，必要时补边到 16 倍数稳定尺寸，返回后再裁回内容尺寸
- 支持长边 `1536 / 2048 / 3072 / 4096`
- 支持超时 `75 / 120 / 180 / 300 / 600` 秒
- 支持官方参数 `text_mode`、`negative_prompt`、`seed`、`cfg_scale` 和 `steps`
- `Seed` 留空时会为每次请求自动生成随机 Seed，并在输出区域显示实际 Seed
- 接口错误会分类显示，并保留原始返回详情，方便区分超时、认证、参数、审核、限流、服务端和网络问题
- 支持处理次数 `1 / 2 / 3 / 4 / 5 / 10`，单张图片和批量文件夹都会按次数重复生成
- 默认使用 `原图优先`、长边 `4096`、超时 `300` 秒
- 显示当前处理图片与输出图片的格式、尺寸、文件大小
- 支持私密模式，隐藏当前图片和结果图片预览
