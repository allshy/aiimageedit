...
界面里的“次数”可以让单张图片或批量文件夹里的每张图片重复处理多次，输出文件名会自动追加 `-01 / -02` 这类序号。

批量处理中也可以点击“停止”，程序会等待当前 API 请求返回或超时后停止后续任务。如果某张图请求超时，会自动跳过这张图剩余的重复次数。
...
- 图片编辑页默认开启“预处理上传”，默认使用 `原图优先`、长边 4096，原图不会被修改。
- 上传模式可选：
  - `原图优先`：如果图片长边不超过 4096，直接原图上传；超过 4096 时才等比缩小到所选长边，不补边。
  - `兼容补边`：遇到接口不稳定或特殊尺寸时使用，会自动识别常见比例，必要时补边到 16 倍数，返回后再裁回内容尺寸。
...
- 也可以直接选择更高步数 `20 / 28 / 50`；步数越高越慢，不一定稳定提升画质。
- 支持官方参数 `text_mode`、`negative_prompt`、`seed` 和 `cfg_scale`。图片里有文字时可勾选“文字模式”；使用负面提示词时建议把 CFG 调到 `3.0` 或更高。
- `Seed` 留空时，程序会为每次请求自动生成一个随机 Seed 并传给接口；输出区域、日志和批量状态会显示本次实际 Seed，方便复现好结果。
- 接口错误会按 `请求超时 / 认证失败 / 权限不足 / 限流或额度不足 / 参数或格式错误 / 平台审核未通过 / 平台服务异常 / 网络连接失败` 分类显示，并保留原始返回详情。
- 超时时间可在界面选择 `75` 到 `600` 秒。批量处理或原图上传建议用 `300` 秒。
- 处理次数可选 `1 / 2 / 3 / 4 / 5 / 10`。批量时每张图片都会处理对应次数。
- 如果批量中某张图超时，程序会记录失败并跳过该图剩余重复次数，避免长时间卡在同一张图上。
...
Android 版已同步单张编辑、批量文件夹处理、重复处理、暂停/继续、私密预览开关、当前/输出图片信息显示、原图优先上传、兼容补边、超时设置和失败清单。
